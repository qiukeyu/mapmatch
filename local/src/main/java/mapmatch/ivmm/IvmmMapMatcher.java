...
import mapmatch.ivmm.Probabilities;
import mapmatch.tihmm.HmmProbabilities;
import mapmatch.tihmm.SequenceState;
import mapmatch.tihmm.TiViterbi;
...
public class IvmmMapMatcher {
    /**
     * emission P 用正态分布函数来模拟，sigma为正态分布的概率函数参数
     */
     ...
     */
    public IvmmMapMatcher(RoadNetwork roadNetwork) {
        this(roadNetwork, ShortestPathAlgoTypeEnum.ASTAR);
    }

    public IvmmMapMatcher(RoadNetwork roadNetwork, ShortestPathAlgoTypeEnum algoType) {
    ...
    private List<SequenceState> computeViterbiSequence(List<STPoint> ptList) {
        ...
        final Probabilities probabilities = new Probabilities(measurementErrorSigma);
     ...
     */
    private void computeEmissionProbabilities(TimeStep timeStep, Probabilities probability) {
        for (CandidatePoint candiPt : timeStep.getCandidates()) {
            final double dist = candiPt.getErrorDistanceInMeter();
            timeStep.addEmissionLogProbability(candiPt, probability.candidatePointLogProbability(dist));
        }
    }
     ...
     */
    protected void computeTransitionProbabilities(TimeStep prevTimeStep, TimeStep timeStep, Probabilities probabilities) {
