...
object MapMatchApp {
  def main(args: Array[String]): Unit = {
    ...
    val chPath = args(3)
    val statPath = args(4)
    val mapmatchAlgorithm = args(5).toInt // 0 for tihmm, 1 for ivmm
    val shortAlgorithm = args(6).toInt
    ...
    val mapMatchRdd = trajRdd.mapPartitions(trajIter => {
      val mapMatcher = if(mapmatchAlgorithm==0) getTihmmMapMatcher(shortAlgorithm, bcRoadSegments.value, bcCacheMap.value, bcCHRoadSegments.value)
      else getIvmmMapMatcher(shortAlgorithm, bcRoadSegments.value, bcCacheMap.value, bcCHRoadSegments.value)
      trajIter.flatMap(traj => Option(mapMatcher.matchTrajToRoute(traj)))
    })
    mapMatchRdd.saveAsTextFile("./dataset/output");
  ...
  def getTihmmMapMatcher(shortAlgorithm: Int, roadSegemnts: Array[RoadSegment],
  ...
  def getIvmmMapMatcher(shortAlgorithm: Int, roadSegemnts: Array[RoadSegment],
                         cacheLines: util.Map[(Integer, Integer), java.lang.Double],
                         chRoadSegments: Array[CHRoadSegment]): TiHmmMapMatcher = {
